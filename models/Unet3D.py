...
class Unet3D(nn.Module):
    def __init__(
        ...
        block_klass1 = partial(ResnetBlock, groups=1)
        block_klass_cond1 = partial(block_klass1, time_emb_dim=cond_dim)
        for ind, (dim_out, dim_in) in enumerate((in_out)):
            dim_out=1
            dim_in=1
            is_last = ind >= (num_resolutions - 1)
            self.inter.append(nn.ModuleList([
                block_klass_cond1(dim_out, dim_in),
                Residual(PreNorm(dim_in, SpatialLinearAttention(
                    dim_in, heads=attn_heads))) if not is_last else nn.Identity(),
                # Residual(PreNorm(dim_in, temporal_attn(dim_in))),
                Upsample_inter(dim_in) if not is_last else nn.Identity()
            ]))
    ...
    def forward(
        ...
        # for  block2, spatial_attn, upsample in self.inter:
        #     x = block2(x, t)
        #     x = spatial_attn(x)
        #     # x = temporal_attn(x, pos_bias=time_rel_pos_bias,
        #     #                   focus_present_mask=focus_present_mask)
